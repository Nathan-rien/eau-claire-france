-- Script de backfill corrigé pour corriger les lignes incomplètes (derniers 3 jours)
DO $$ 
BEGIN
    -- Mettre à jour unit_volume_l depuis product_name
    ...
    WHERE scraped_at > now() - interval '3 days'
      AND unit_volume_l IS NULL;

    -- Mettre à jour pack_count depuis product_name
    UPDATE public.prices_history
    SET pack_count = 
        COALESCE(pack_count,
