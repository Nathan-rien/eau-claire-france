...
export const calculateNutritionalScore = (bottle: BottleWaterData): BottleRanking => {
  ...
  // Access composition properties correctly - they should be direct properties on BottleWaterData
  const composition = bottle.composition || bottle;

  // Score pour les nitrates (0-10 points, plus c'est faible mieux c'est)
  if (composition.nitrates < 5) scores.nitrates = 10;
  else if (composition.nitrates <= 10) scores.nitrates = 6;
  else scores.nitrates = 2;

  // Score pour le résidu sec (0-10 points)
  if (composition.residusSec >= 150 && composition.residusSec <= 500) scores.residuSec = 10;
  else if (composition.residusSec > 500 && composition.residusSec <= 1500) scores.residuSec = 6;
  else scores.residuSec = 2;

  // Score pour le calcium (0-10 points)
  if (composition.calcium >= 150) scores.calcium = 10;
  else if (composition.calcium >= 50) scores.calcium = 6;
  else scores.calcium = 2;

  // Score pour le magnésium (0-10 points)
  if (composition.magnesium > 50) scores.magnesium = 10;
  else if (composition.magnesium >= 20) scores.magnesium = 6;
  else scores.magnesium = 2;

  // Score pour le sodium (0-10 points, plus c'est faible mieux c'est)
  if (composition.sodium < 20) scores.sodium = 10;
  else if (composition.sodium <= 100) scores.sodium = 6;
