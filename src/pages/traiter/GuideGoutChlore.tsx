...
export default function GuideGoutChlore() {
  ...
  const FAQ = [1, 2, 3, 4].map((i) => ({
    q: t(`sat.chlore.faq.q${i}.q`),
    a: t(`sat.chlore.faq.q${i}.a`),
  }));

  const solutions = [1, 2, 3, 4].map((i) => ({
    title: t(`sat.chlore.sol.${i}.t`),
    desc: t(`sat.chlore.sol.${i}.d`),
  }));

  const links = [
    { to: "/traiter-eau-robinet", title: t("sat.chlore.links.pillar"), desc: t("sat.chlore.links.pillar.desc") },
    { to: "/comparatif-carafes", title: t("sat.chlore.links.carafes"), desc: t("sat.chlore.links.carafes.desc") },
    { to: "/guide/quel-filtre-eau", title: t("sat.chlore.links.filter"), desc: t("sat.chlore.links.filter.desc") },
    { to: "/guide/eau-calcaire", title: t("sat.chlore.links.calcaire"), desc: t("sat.chlore.links.calcaire.desc") },
    { to: "/diagnostic", title: t("sat.chlore.links.diagnostic"), desc: t("sat.chlore.links.diagnostic.desc") },
    { to: "/qualite-eau", title: t("sat.chlore.links.quality"), desc: t("sat.chlore.links.quality.desc") },
  ];
  ...
  const schemas = [
    {
      ...
      "@type": "Article",
      headline: t("sat.chlore.h1"),
      description: t("sat.chlore.seoDesc"),
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: t("sat.chlore.breadcrumb.guides"), item: "https://infoeau.fr/traiter-eau-robinet" },
        { "@type": "ListItem", position: 3, name: t("sat.chlore.breadcrumb.current"), item: `https://infoeau.fr${CANONICAL}` },
      ],
    },
  ];
  ...
  return (
    <Layout>
      <SEOHead
        title={t("sat.chlore.seoTitle")}
        description={t("sat.chlore.seoDesc")}
        canonical={CANONICAL}
        keywords="goût de chlore eau, enlever chlore eau robinet, odeur de chlore, carafe charbon actif, eau du robinet goût"
        schemaData={schemas}
      />
          ...
          <div className="container mx-auto max-w-5xl px-4 py-12 md:py-16 lg:py-20">
            <nav className="text-sm text-muted-foreground mb-6" aria-label="Breadcrumb">
              ...
              <Link to="/traiter-eau-robinet" className="hover:text-foreground">
                {t("sat.chlore.breadcrumb.guides")}
              </Link>
              <span className="mx-2">›</span>
              <span className="text-foreground">{t("sat.chlore.breadcrumb.current")}</span>
            </nav>

            <div className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-blue-700 mb-4">
              <Droplets className="w-4 h-4" />
              <span>{t("sat.chlore.tag")}</span>
            </div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Button asChild>
                <Link to="/diagnostic">
                  {t("sat.chlore.cta.diag")}
                  <ArrowRight className="w-4 h-4 ml-1" />
                </Link>
              </Button>
              <Button asChild variant="outline">
                <Link to="/qualite-eau">{t("sat.chlore.cta.quality")}</Link>
              </Button>
            </div>
            ...
            <div className="mt-8 flex items-start gap-3 rounded-xl bg-white/70 backdrop-blur px-4 py-3 text-sm text-muted-foreground ring-1 ring-black/5 max-w-3xl">
              ...
              <p className="m-0">{t("sat.chlore.note")}</p>
            </div>
          </div>
        </section>
        ...
        <section className="px-4 py-12 md:py-16">
          <div className="container mx-auto max-w-4xl">
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-6 tracking-tight">
              {t("sat.chlore.concerned.title")}
            </h2>
            <p className="text-muted-foreground leading-relaxed mb-4">{t("sat.chlore.concerned.p1")}</p>
            <p className="text-muted-foreground leading-relaxed">{t("sat.chlore.concerned.p2")}</p>
            <div className="mt-6 flex flex-wrap gap-4 text-sm font-medium">
              <Link to="/qualite-eau" className="inline-flex items-center gap-1 text-blue-700 hover:underline">
                {t("sat.chlore.links.quality")}
                <ArrowRight className="w-4 h-4" />
              </Link>
              <Link to="/diagnostic" className="inline-flex items-center gap-1 text-blue-700 hover:underline">
                {t("sat.chlore.links.diagnostic")}
                <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
          </div>
        ...
        <section className="px-4 py-12 md:py-16 bg-muted/30 border-y border-border">
          <div className="container mx-auto max-w-4xl">
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4 tracking-tight">
              {t("sat.chlore.sol.title")}
            </h2>
            <p className="text-muted-foreground leading-relaxed mb-8">{t("sat.chlore.sol.intro")}</p>
        ...
        <section className="px-4 py-12 md:py-16">
          <div className="container mx-auto max-w-4xl">
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-6 tracking-tight flex items-center gap-3">
              ...
              {t("sat.chlore.limits.title")}
            </h2>
            <p className="text-muted-foreground leading-relaxed mb-4">{t("sat.chlore.limits.p1")}</p>
            <p className="text-muted-foreground leading-relaxed mb-4">{t("sat.chlore.limits.p2")}</p>
